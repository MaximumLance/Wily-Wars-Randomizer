...
with open(RandomizedROM, "rb+") as ROM:
    ...
    Pointer = 0x85A1C #Checking to see if User wants Random Wily Stages
    Seek = ROM.seek(Pointer,0)
    Value = ROM.read(1)
    if Value == Flag5:
        Pointer = 0x85A1D
    ...
    Pointer = 0x85A1E #Checking to see if User wants to skip Wily Castle Intro
    Seek = ROM.seek(Pointer,0)
    Value = ROM.read(1)
    NoSkip2 = False #If user doesn't want to skip MM2 Wily Castle Intro cutscene
    if Value == Flag5:
        Skip = [b'\x4E',b'\xB9',b'\x00',b'\x08',b'\x56',b'\xD0']
        Pointer = 0x85A1F
        Seek = ROM.seek(Pointer,0)
        Value = ROM.read(1)
        if Value == Flag6:
            Pointer = 0x13204
            Seek = ROM.seek(Pointer,0)
            for x in range(6):#Writing values to ROM if so
                Value = Skip[x]
                ROM.write(Value)
        elif Value != Flag6:
            if MM2 == True:
...
#Writes boss value to stage address based on bosses chosen
        ...
        for x in range(8): #!
            if posB[x][1] == x: #This condition is unneccessary.
                if posB[x][0] == "Cutman":
                    ...
                    Seek = ROM.seek(Pointer,0)
                    ROM.write(Bosses[x])
                    if Pointer == 0x7AC63: # If MM1 Boss is in Hardman, Needleman, or Magnetman stage, adjust vertical position
                        Pointer += 2
                        Seek = ROM.seek(Pointer, 0)
                        ROM.write(b'\xD4')
                    elif Pointer == 0x7AE53 or Pointer == 0x7B4A9:
                        Pointer += 2
                        Seek = ROM.seek(Pointer, 0)
                        ROM.write(b'\xB4')
        ...
        if RearrangeMe == True: #! If Metalman is a boss in Hard, Gemini, Magnet, or Shadow, replace him with another boss
            for x in range(8):
                if x != Posi:
                    ...
                    if Counter == 0:
                        PosiA = x
                        break
            Pointer = Stageaddress[PosiA] #Saving boss value in Stage 1 for swap then writing Metalman value
            Seek = ROM.seek(Pointer,0)
            Value = ROM.read(1)
            Seek = ROM.seek(Pointer,0)
            ROM.write(MetalV)
            ...
            String = posB[PosiA][0] #Exchange posB values to accurately reflect which boss is where
            String2 = posB[Posi][0]
            posB.pop(PosiA)
            posB.insert(PosiA, [String2, PosiA]) #Inserts the string at PosiA and the value of PosiA to keep the same format of [Boss, X]
            posB.pop(Posi)
        ...
        if RearrangeMa == True:
            ...
            posB.pop(PosiB)
            posB.insert(PosiB, [String2, PosiB])
            posB.pop(Posi2)
        ...
        if RearrangeCu == True:
            ...
            String = posB[PosiC][0]
            String2 = posB[Posi3][0]
            posB.pop(PosiC)
            posB.insert(PosiC, [String2, PosiC])
            posB.pop(Posi3)
            posB.insert(Posi3, [String, Posi3])
...
#Checks to see if player will receive Time Stopper
    ...
    if Flash == True:
        ...
        Stopper -= 1
        if Vanilla == True or randomweapons == True:
...
#Intializes default values to FF
    ...
    if GetMarine == "Cutman":
        Pointer = 0x664B3
        MarineFlag.append(["Cutman", GiveMarine])
    elif GetMarine == "Gutsman":
        Pointer = 0x664B5
        MarineFlag.append(["Gutsman", GiveMarine])
    elif GetMarine == "Iceman":
        Pointer = 0x664B7
        MarineFlag.append(["Iceman", GiveMarine])
    elif GetMarine == "Bombman":
        Pointer = 0x664B9
        MarineFlag.append(["Bombman", GiveMarine])
    elif GetMarine == "Fireman":
        Pointer = 0x664BB
        MarineFlag.append(["Fireman", GiveMarine])
    elif GetMarine == "Elecman":
        Pointer = 0x664BD
        MarineFlag.append(["Elecman", GiveMarine])               
    elif GetMarine == "Flashman": #If boss was X, sets address pointer to the address that will give them an item
        Pointer = 0x664E7
...
#Writes RushJet value to randomly chosen boss
    ...
    if GetJet == "Cutman": 
        Pointer = 0x664B3
        JetFlag.append(["Cutman", GiveJet])
    elif GetJet == "Gutsman":
        Pointer = 0x664B5
        JetFlag.append(["Gutsman", GiveJet])
    elif GetJet == "Iceman":
        Pointer = 0x664B7
        JetFlag.append(["Iceman", GiveJet])
    elif GetJet == "Bombman":
        Pointer = 0x664B9
        JetFlag.append(["Bombman", GiveJet])
    elif GetJet == "Fireman":
        Pointer = 0x664BB
        JetFlag.append(["Fireman", GiveJet])
    elif GetJet == "Elecman":
        Pointer = 0x664BD
        JetFlag.append(["Elecman", GiveJet])     
    elif GetJet == "Flashman":
        Pointer = 0x664E7
    ...
    if MM2 == True: #!
        if GetI3 == "Cutman": 
            Pointer = 0x664B3
            I3Flag.append(["Cutman", GiveI3])
        elif GetI3 == "Gutsman":
            Pointer = 0x664B5
            I3Flag.append(["Gutsman", GiveI3])
        elif GetI3 == "Iceman":
            Pointer = 0x664B7
            I3Flag.append(["Iceman", GiveI3])
        elif GetI3 == "Bombman":
            Pointer = 0x664B9
            I3Flag.append(["Bombman", GiveI3])
        elif GetI3 == "Fireman":
            Pointer = 0x664BB
            I3Flag.append(["Fireman", GiveI3])
        elif GetI3 == "Elecman":
            Pointer = 0x664BD
            I3Flag.append(["Elecman", GiveI3])     
        elif GetI3 == "Flashman": #If boss was X, sets address pointer to the address that will give them an item
            Pointer = 0x664E7
...
#!Wily Refight Capsule boss writing section
    ...
    if MM3 == True or Wily3 == True: #!
        ...
        if FlatSp4Arena == True:
            ...
            Seek = ROM.seek(Pointer, 0)
            ROM.write(b'\xBA')
            Pointer = 0x7C863 #Set Teleporter y pos to match flat arena
            Seek = ROM.seek(Pointer, 0)
            ROM.write(b'\xB0')
        ...
        if FlatSn4Arena == True: # Set Address Pointer to Flat Arena
            Pointer = 0x16D860
            Seek = ROM.seek(Pointer, 0)
            ROM.write(b'\xE3')
            Pointer += 1
...
#Writing Gutsman Pointers and other related data
    ElecStage = b'\x05'
    CutStage = b'\x00'
    BombStage = b'\x03'
    FireStage = b'\x04'
    ...
    MagnetStage = b'\x2E'
    ShadowStage = b'\x2F'
    MM2Pointers = [0x7A3AB,0x7A3AF,0x7A3B3,0x7A3B7,0x7A3BB,0x7A3BF,0x7A3C3,0x7A3C7]
    MM3Pointers = [0x7C70D,0x7C711,0x7C715,0x7C719,0x7C71D,0x7C721,0x7C725,0x7C729]
    Position = 0
    if Gutsman == True: #!
        Telebit = 0
        for x in range(8):
            if Bossorder[x] == "Gutsman": #Determining which capsule Gutsman is in
                Position = x
                break

        if MM2 == True or Wily2 == True:
            Pointer = MM2Pointers[Position]
            Seek = ROM.seek(Pointer,0) #Writing Refight pointer
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xE5')
            Position += 2
            Telebit = Position
            Telebit2 = Telebit.to_bytes(1, 'big')
            Seek = ROM.seek(0x85AEA,0) #Writing MM2 specific teleporter data
            ROM.write(Telebit2)

        if MM3 == True or Wily3 == True:
            Pointer = MM3Pointers[Position] #Writing Refight pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xE5')
            Position += 3 #Writing specific data to determine which teleporter Gutsman was in
            Telebit = Position
            Telebit2 = Telebit.to_bytes(1, 'big')
            Seek = ROM.seek(0x85AE9, 0)
            ROM.write(b'\x03')
            ROM.write(Telebit2)
            ROM.write(b'\x10')
            if Pointer == 0x7C715: #Adjusting Needleman arena teleporter y pos
                Seek = ROM.seek(0x85AE8, 0)
                ROM.write(b'\xC0')
    ...
    if Vanilla == False:
        for x in range(8):
            if posB[x][0] == "Gutsman":
                Position = x
                break
        if Stages[Position] == ElecStage:
            Pointer = 0x78671 #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            Pointer = 0x85AC9 #Adjusting door for vertical opening
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x80')
            ROM.write(b'\x10')
            ROM.write(b'\x00')
            ...
            ROM.write(b'\x13')
            ROM.write(b'\x13')
        elif Stages[Position] == CutStage:
            Pointer = 0x77BAF #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
        elif Stages[Position] == IceStage:
            Pointer = 0x77E6B #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
        elif Stages[Position] == FireStage:
            Pointer = 0x78431 #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
        elif Stages[Position] == BombStage:
            Pointer = 0x78221 #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            ROM.write(b'\x13')
            ROM.write(b'\x13')
        elif Stages[Position] == BubbleStage:
            Pointer = 0x7901F #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            Pointer = 0x85ACB #Adjusting door y pos
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x40')
        elif Stages[Position] == AirStage:
            Pointer = 0x791AB #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x40')
        elif Stages[Position] == QuickStage:
            Pointer = 0x79317 #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x40')
            Pointer = 0x85AD2 #Adjusting boss x/y pos
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\xC0')
            ROM.write(b'\xA0')
        elif Stages[Position] == HeatStage:
            Pointer = 0x795F7 #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x40')
        elif Stages[Position] == WoodStage:
            Pointer = 0x79877 #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x40')
        elif Stages[Position] == MetalStage:
            Pointer = 0x79A2F #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x40')
        elif Stages[Position] == FlashStage:
            Pointer = 0x79B7B #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            ROM.write(b'\xC4')
            ROM.write(b'\x8D')
        elif Stages[Position] == CrashStage:
            Pointer = 0x79CF3 #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x40')
        elif Stages[Position] == SparkStage:
            Pointer = 0x7A65B #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x80')
        elif Stages[Position] == SnakeStage:
            Pointer = 0x7A85B #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x80')
        elif Stages[Position] == NeedleStage:
            Pointer = 0x7AB2B #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            Pointer = 0x85AD3 #Adjusting boss y pos
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\xCE')
        elif Stages[Position] == HardStage:
            Pointer = 0x7ACC9 #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x80')
        elif Stages[Position] == TopStage:
            Pointer = 0x7AEB1 #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x40')
        elif Stages[Position] == GeminiStage:
            Pointer = 0x7B0A5 #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x80')
        elif Stages[Position] == MagnetStage:
            Pointer = 0x7B34D #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
            ...
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x20')
        elif Stages[Position] == ShadowStage:
            Pointer = 0x7B50B #Writing room object pointer
            Seek = ROM.seek(Pointer,0)
            ROM.write(b'\x08')
            ROM.write(b'\x5A')
            ROM.write(b'\xC8')
...
#Random Wily Stage Error correction section
    Werror = []
    Extras = [b'\x1D',b'\x1F',b'\x07',b'\x37',b'\x36']
    Error = False
    if Wily == True: #!
        Pointer = 0x857FA
        Seek=ROM.seek(Pointer,0)
        ...
        for x in range(4):
            Value = Werror[x]
            y = Werror.count(Value)
            if y == 2:
                Error = True
                z = Werror.index(Value)
                for w in range(5):
                    ValueA = Extras[w]
                    a = Werror.count(ValueA)
                    if a == 0:
                        break
                Werror.pop(z)
                Werror.append(ValueA)
                break
            elif y == 3:
                Error = True
                for b in range(2):
    ...
    Bombweak = [0x6E77F,0x6E780,0x6E781,0x6E782,0x6E783,0x6E784,0x6E785,0x6E786,0x6E787,0x6E788,
                0x6E789,0x6E78A,0x6E78B,0x6E78C,0x6E78D,0x6E78E,0x6E78F,0x6E790,0x6E791,0x6E792,
                0x6E793,0x6E794,0x6E795]
    ...
    Wily3Weak = [0x6F333,0x6F334,0x6F335,0x6F336,0x6F337,0x6F338,0x6F339,0x6F33A,0x6F33B,0x6F33C,
    ...
    Wily2Weak = [0x6ED9F,0x6EDA0,0x6EDA1,0x6EDA2,0x6EDA3,0x6EDA4,0x6EDA5,0x6EDA6,0x6EDA7,0x6EDA8,
    ...
    YellowDevilWeak = [0x6E80B,0x6E80C,0x6E80D,0x6E80E,0x6E80F,0x6E810,0x6E811,0x6E812,0x6E813,
                    0x6E814,0x6E815,0x6E816,0x6E817,0x6E818,0x6E819,0x6E81A,0x6E81B,0x6E81C,0x6E81D,
                  0x6E81E,0x6E81F,0x6E820,0x6E821]

    CopyWeak = [0x6E87B,0x6E87C,0x6E87D,0x6E87E,0x6E87F,0x6E880,0x6E881,0x6E882,0x6E883,
                    0x6E884,0x6E885,0x6E886,0x6E887,0x6E888,0x6E889,0x6E88A,0x6E88B,0x6E88C,0x6E88D,
                  0x6E88E,0x6E88F,0x6E890,0x6E891]

    BMachineWeak = [0x6E85F,0x6E860,0x6E861,0x6E862,0x6E863,0x6E864,0x6E865,0x6E866,0x6E867,
                    0x6E868,0x6E869,0x6E86A,0x6E86B,0x6E86C,0x6E86D,0x6E86E,0x6E86F,0x6E870,0x6E871,
                  0x6E872,0x6E873,0x6E874,0x6E875]

    WMachineWeak = [0x6E827,0x6E828,0x6E829,0x6E82A,0x6E82B,0x6E82C,0x6E82D,0x6E82E,0x6E82F,
                    0x6E830,0x6E831,0x6E832,0x6E833,0x6E834,0x6E835,0x6E836,0x6E837,0x6E838,0x6E839,
                  0x6E83A,0x6E83B,0x6E83C,0x6E83D]
    ...
    Pointers = [0x85810,0x85820,0x85830,0x85840,0x85850,0x85860,0x85870,0x85880,
                ...
                0x85910,0x85920,0x85930,0x85940,0x85960,0x85970,0x85988,0x85998]
    ...
    Pointers2 = []
    if Vanilla == True: #!
       for x in range(8):#Seeing which boss is encountered
            if pos[x][0] == "Bombman":
                Offset = 0
                Weak.append(Offset)
...
#Seeing which boss is encountered
    ...
    if Wily == False:
        ...
        if MM3 == True:
            for x in range(16):
                Pointers2.append(Pointers[x])
        elif MM2 == True:
            for x in range(8):
                Pointers2.append(Pointers[x])
        WilyS = []
        Pointer = 0x857FA #Gets random wily stage value from addresses
        Seek = ROM.seek(Pointer,0)
        for x in range(6):
            Value = ROM.read(1)
            WilyS.append(Value)
        for x in range(6):
            if WilyS[x] == b'\x06':
                Weak2.append(YellowDevilWeak)
                Pointers2.append(0x85828)
            elif WilyS[x] == b'\x07':
                if WilyA1 == False:
                    Weak2.append(Cutweak)
                    Pointers2.append(0x85810)
                    Extras += 1
                if WilyA2 == False:
                    Weak2.append(Elecweak)
                    Pointers2.append(0x85870)
                    Extras += 1
                Weak2.append(CopyWeak)
                Pointers2.append(0x85838)
            elif WilyS[x] == b'\x08':
                Weak2.append(BMachineWeak)
                Pointers2.append(0x85848)
            elif WilyS[x] == b'\x09':
                if WilyB1 == False:
                    Weak2.append(Bombweak)
                    Pointers2.append(0x858A0)
                    Extras += 1
                if WilyB2 == False:
                    Weak2.append(Fireweak)
                    Pointers2.append(0x85840)
                    Extras += 1
                if WilyB3 == False:
                    Weak2.append(Iceweak)
                    Pointers2.append(0x85830)
                    Extras += 1
                if WilyB4 == False:
                    Weak2.append(Gutsweak)
                    Pointers2.append(0x858C0)
                    Extras += 1
                Weak2.append(WMachineWeak)
                Pointers2.append(0x85858)
            elif WilyS[x] == b'\x1C':
                Weak2.append(MechaDWeak)
                Pointers2.append(0x85970)
                Wily21 = True
            elif WilyS[x] == b'\x1D':
                Weak2.append(PicoWeak)
                Pointers2.append(0x85910)
                Wily22 = True
            elif WilyS[x] == b'\x1E':
                Weak2.append(GutsTankWeak)
                Pointers2.append(0x85890)
            elif WilyS[x] == b'\x1F':
                Weak2.append(TrapWeak)
                Pointers2.append(0x85988)
                Wily4 = True
            elif WilyS[x] == b'\x20':
                Weak2.append(Wily2Weak)
                Pointers2.append(0x858A0)
                ...
                Weak2.append(YellowWeak)
                Pointers2.append(0x85920)
            elif WilyS[x] == b'\x37':
                Weak2.append(MegaWeak)
                Pointers2.append(0x85930)
            elif WilyS[x] == b'\x39':
                Weak2.append(Wily3Weak)
                Pointers2.append(0x85940)
            elif WilyS[x] == b'\x3A':
                Weak2.append(Gamma1Weak)
                Weak2.append(Gamma2Weak)
                Pointers2.append(0x85960)
                Pointers2.append(0x85970)
                Extras += 1
                
    if MM2 == True or Wily4 == True or Wily6 == True:
    #!Writing weakness values for Trap and Alien bosses
        ...
        Pointer = 0x85988 #Write values to ROM based on random values
        Seek = ROM.seek(Pointer,0)
        for x in range(8):
            if x == Trap1:
                ROM.write(FlagW)
...
#!Weapon received writing if randomboss
    ...
    if Vanilla == True:
        for x in range(8):
            if pos[x][0] == "Cutman": #If boss is present, write it's weapon value based on it's position
                Pointer = Weaponget[x]
                Seek = ROM.seek(Pointer, 0)
                ROM.write(b'\x05')
            elif pos[x][0] == "Gutsman":
...
#MM2 mode specific writing
    ...
    MM3W = [b'\x35',b'\x36',b'\x37',b'\x38',b'\x39',b'\x3A']
    MM2W = [b'\x1C',b'\x1D',b'\x1E',b'\x1F',b'\x20',b'\x21']
    if Wily == False: #! If not randomizing wily stages, writes correct stages here.
        Pointer = 0x857FA
        Seek =ROM.seek(Pointer,0)
        for x in range(6):
            if MM3 == True:
    ...
    if Wily == True: #Stage editing if MM3 and random wily stage is MM2 Wily 1 or Wily 2
        if MM3 == True:
            if Wily21 == True:
                Pointer = 0x129F60
                Seek = ROM.seek(Pointer,0)
            ...
            if Wily22 == True:
                Value = b'\x84'
                Value2 = [b'\x8B',b'\x8A',b'\x85',b'\x86',b'\x85',b'\x8C']
                Pointer = 0x12B3A5
                Seek = ROM.seek(Pointer,0)
                for x in range(4):
                    ROM.write(Value)
            ...
            Pointer = 0x857FA #Light and Wily Capsule Cutscene enable
            Seek = ROM.seek(Pointer,0)
            Value = ROM.read(1)
            Pointer = 0x1BA4D
            Seek = ROM.seek(Pointer,0)
            ...
            Pointer = 0x857FF #Secret Skull enable on last stage
            Seek = ROM.seek(Pointer,0)
            Value = ROM.read(1)
            Pointer = 0x1AB75
            Seek = ROM.seek(Pointer,0)
    ...
    if NoSkip2 == True:
        Pointer = 0x857FA #Wily Capsule Cutscene enable
        Seek = ROM.seek(Pointer,0)
        Value = ROM.read(1)
        Pointer = 0x1B0C9
        Seek = ROM.seek(Pointer,0)
