...
with open(RandomizedROM, "rb+") as ROM:
    ...
    randomdamage = False
    randomammo = False
    Pointer = 0x85A05 #Checks to see if user wants Random Buster,Damage from Bosses, or Ammo Usage
    Seek = ROM.seek(Pointer,0)
    Value = ROM.read(1)
    Flag5 = b'\x00'
    Flag6 = b'\x01'
...
#This is the seed value writing section.        
    if len(Seed) <= 7: 
        ...
        Pointer = [0x199B5F,0x199B65,0x199B67,0x199B6D,0x199B6F,0x199B75,0x199B77] #Addresses for seed value
        for x in range(len(Seed)): #Writes values to addresses
            Seek = ROM.seek(Pointer[x],0)
            ROM.write(Seedbyte[x])
    else:
        print("Seed value is more than 7.")
...
#This is the Boss encountered list writing if no randomboss    
    ...
    Pointer = 0x644C0
    Seek = ROM.seek(Pointer,0)
    for x in range(8): #Adds values from x644C0-7 and joins them to the Stages list
        Byte = ROM.read(1)
        Stages.append(Byte)
        Pointer+=1
    Error = False
    ...
    if rearrange == True: #If a MM3 stage was found, rebuilds the pos list
            ...
            for y in range(2):
                for x in range(8): #Runs through this loop twice for accuracy on proper positioning
                    if pos2[x] == "Sparkman":
                        pos2.pop(x)
...
#Wily 4 Capsule boss writing section
    ...
    for x in range(4): #If MM1 boss is in Shadow,Hard, Needle, or Magnet capsules, asjust vertical positioning
        Pointer = Pointer2[x]
        Seek = ROM.seek(Pointer, 0)
        Value = ROM.read(1)
        if Value == b'\x14':
...
#Writes randomly generated weapon values to a list and then writes the You Got Weapon text                
    ...
    if randomweapons == True: 
        ...
        if Vanilla == False: #Condition statement doesn't need to be here, but I didn't want to unindent everything below.
            Pointer = 0x6482A
...
#Get weapon screen if randomboss/weapons and palette writing if random weapons
    if Vanilla == False:
        for x in range(8):
...
#Weakness writing section    
    ...
    Wily1Weak = [0x6F333,0x6F334,0x6F335,0x6F336,0x6F337,0x6F338,0x6F339,0x6F33A,0x6F33B,0x6F33C,
                0x6F33D,0x6F33E,0x6F33F,0x6F340,0x6F341,0x6F342,0x6F343,0x6F344,0x6F345,0x6F346,
                 0x6F347,0x6F348,0x6F349]
    
    Wily2Weak = [0x6F873,0x6F874,0x6F875,0x6F876,0x6F877,0x6F878,0x6F879,0x6F87A,0x6F87B,0x6F87C,
    ...
    Pointers = [0x85810,0x85820,0x85830,0x85840,0x85850,0x85860,0x85870,0x85880,
                ...
                0x85910,0x85920,0x85930,0x85940,0x85950,0x85960,0x85970]
...
#Weapon received writing if randomboss
    ...
    #Writes FF to Weapon Recieved address if MM3 stage is not a stage
